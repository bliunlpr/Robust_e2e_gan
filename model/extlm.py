...
class NgramCharacterKenLM():
    ...
    def predict(self, state, x):
        ...
        yseq_prev = state
        kenlm_score = [self.compute_score(yseq_prev, str(int(hyp))) for hyp in x]
        state = {'yseq_prev': yseq_prev, 'kenlm_score_prev': kenlm_score}
        return state, np.array(kenlm_score, dtype=np.float32)
        
    def compute_score(self, prev, hyp):
        prev = prev[-(self.order - 1):]
        prev = ' '.join(list(map(str, prev)))
        all = prev + ' ' + hyp
        return self.model.score(all, bos=False, eos=False) - self.model.score(prev, bos=False, eos=False)    
