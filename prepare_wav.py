...
def make_wav(path_list, enhanced_wav_dir):
    for path in path_list:
        ...
        ##result = 1j * image
        ##result += real 
        result = feat_mat * np.exp(1j * angle_mat)
        D = np.array(librosa.istft(result.T, hop_length=256, win_length=512, window=scipy.signal.hamming))
        ##D = D / np.max(D)
        wav_file = os.path.join(enhanced_wav_dir, uttid + '.wav')
        if not os.path.exists(wav_file):
            wav.write(wav_file, 16000, np.int16(D))
...
def main():
    ...
    ##make_wav(enhanced_list, enhanced_wav_dir)
