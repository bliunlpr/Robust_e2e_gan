...
def main():
    ...
    if opt.resume:
        ...
        if os.path.isfile(model_path):
            ...
            fbank_model = E2E.FbankModel(model_path, 'fbank_state_dict') 
            logging.info('Loading model {} and iters {}'.format(asr_model_path, opt.iters))
        else:
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=0):
            ...
            iters += 1
            errors = {'train/loss': loss.item(), 'train/acc': acc}
            if opt.mtlalpha == 0.0:
                errors['train/loss_att'] = loss_att.item()
            elif opt.mtlalpha == 1.0:
                errors['train/loss_ctc'] = loss_ctc.item()
            else:  
                errors['train/loss_att'] = loss_att.item()
                errors['train/loss_ctc'] = loss_ctc.item()   
            ...
            if iters % opt.print_freq == 0:
                ...
                state = {'asr_state_dict': asr_model.state_dict(), 
                         'fbank_model': fbank_model.state_dict(), 
            ...
            if iters % opt.validate_freq == 0:
                ...
                for i, (data) in tqdm(enumerate(val_loader, start=0)):
                    utt_ids, spk_ids, inputs, log_inputs, targets, input_sizes, target_sizes
                    fbank_features = fbank_model(inputs, fbank_cmvn)
                    loss_ctc, loss_att, acc = asr_model(fbank_features, targets, input_sizes, target_sizes, sche_samp_rate) 
                    if opt.mtlalpha == 0.0:
                        loss = loss_att
                        errors['val/loss_att'] = loss_att.item()
                    elif opt.mtlalpha == 1.0:
                        loss = loss_ctc   
                        errors['val/loss_ctc'] = loss_ctc.item() 
                    else:
                        loss = opt.mtlalpha * loss_ctc + (1 - opt.mtlalpha) * loss_att  
                        errors['val/loss_att'] = loss_att.item()
                        errors['val/loss_ctc'] = loss_ctc.item() 
