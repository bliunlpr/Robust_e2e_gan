...
if [ ! -d $s1 ]; then
  ...
  for f in utt2spk spk2utt spk2warp feats.scp text wav.scp cmvn.scp spk2gender \
    vad.scp segments reco2file_and_channel utt2lang text_syllable; do
...
for f in feats.scp text_char text_word text_syllable vad.scp utt2lang $maybe_wav_scp utt2dur utt2num_frames; do
  if [ -f $data/$f ]; then
