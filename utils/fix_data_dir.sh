...
for x in utt2spk spk2utt feats.scp text_char text_word segments wav.scp cmvn.scp vad.scp \
...
function filter_utts {
  ...
  for x in feats.scp text_char text_word segments utt2lang $maybe_wav; do
    if [ -f $data/$x ]; then
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text_char text_word segments utt2lang utt2dur utt2num_frames $maybe_wav $utt_extra_files; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
